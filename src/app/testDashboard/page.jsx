...
function Admin() {
  ...
  useEffect(() => {
    const fetchMyProducts = async () => {
      try {
        setIsLoading(true);
        await axios.post("http://localhost:3000/api/getMyProducts", userData).then((response) => (
  ...
  const tableHeading = [
    ...
    },
    {
      ...
      cell: row => (
        <div className='flex justify-between'>
          <button onClick={() => editProduct(row)} className='text-green-500 text-[1.5rem]'><FaRegEdit /></button>
          <button onClick={() => deleteProduct(row)} className='text-red-500 text-[1.5rem] ml-5'>{isLoading ? <ClipLoader /> : <MdDelete />}</button>
        </div>
      ),
    }
  ];
  ...
  const onDrop = (acceptedFiles) => {
    ...
    reader.onload = () => {
      if (productEditMode) {
        ...
        setUserUpdatedData({
          ...userUpdatedData, profileImage: reader.result
        })
  ...
  const getUserId = async () => {
    setUserUpdatedData({
      ...userUpdatedData, userId: userData._id
    })
  };
  ...
  const updateEditedUser = async () => {
    ...
    if (userUpdatedData.userId == undefined) {
      await getUserId();

      if (userUpdatedData.userId !== undefined) {
        setIsLoading(true)
        axios.post("http://localhost:3000/api/updateEditedUser", userUpdatedData, { tempId }).then((response) => (
          setIsLoading(false),
          toast.success(response.data.message),
    ...
    <div className='flex flex-col'>
      ...
      <div className={`md:hidden ${menuButton ? "block" : "hidden"} absolute top-[3.45rem] z-50 w-full h-screen text-center  bg-[#002147] pt-[1rem] leading-[3.5rem]   pb-6`}>
        <ul className='text-white mt-4 flex flex-col space-y-4'>
          ...
          <li onClick={() => (setMenuButton((prev) => !prev), setAdminButton("addNewProduct"))} className={`text-[1.3rem] font-serif text-white ${adminButton === "addNewProduct" ? 'border-b-4 w-[8rem] ml-[8rem] border-[#FFB606]' : ''}`}>Add Product</li>
          ...
          <li onClick={() => (setMenuButton((prev) => !prev), setAdminButton("allProducts"))} className={`text-[1.3rem] font-serif text-white ${adminButton === "allProducts" ? 'border-b-4 w-[10rem] ml-[7rem] border-[#FFB606]' : ''}`}>See All Products</li>
          ...
          <ul className=" mt-9 ">
            <li className={` ml-10 flex`}>
              <button onClick={() => setUserButton(prev => !prev)} className='flex items-center'>
                ...
                <span className="text-[1rem] flex text-white ml-2 rounded-2xl">{isLoading ? <ClipLoader color='white' /> : userData.name}<div className='mt-1'>{userButton ? <IoIosArrowUp /> : <IoIosArrowDown />}</div></span>
              </button>
              <div className='flex ml-16'>
                <div className='text-[#FFB606] text-[1.4rem] mt-1'><Link href='/cart'><FaCartShopping /></Link></div>
                <div className={`relative top-4 right-1 ${cart.length === 0 ? "hidden" : ""} w-4 h-4 text-center font-bold text-[0.7rem] text-[#002147] bg-[#FFB606] rounded-full`}>{cart.length}</div>
            ...
            <li className={`font-serif font-semibold text-[1.3rem] hover:text-[#FFB606] border-b-[1px] mt-[3rem] ml-10 ${adminButton === "addNewProduct" ? "text-[#FFB606]" : " "}`}><button onClick={() => (setAdminButton("addNewProduct"))}>Add New Product</button></li>
            ...
            <li className={`font-serif font-semibold text-[1.3rem] hover:text-[#FFB606] border-b-[1px] mt-[6rem] ml-10 ${adminButton === "allProducts" ? "text-[#FFB606]" : " "}`}><button onClick={() => (setAdminButton("allProducts"))}>See All Products</button></li>
            {/* <li className={`font-serif font-semibold text-[1.3rem] text-[#002147] bg-[#FFB606] rounded-full border-2 border-[#002147] hover:border-2 hover:border-white text-center  mt-[11rem] ml-10`}><button onClick={userLogout}>{logoutLoading ? <ClipLoader /> : "Logout"}</button></li> */}
            {userData && (
              <div className={`absolute ${!userButton ? "hidden" : ""} top-[2rem] left-[6rem]  z-10 mt-9 w-48  rounded-md bg-white shadow-lg`}>
        ...
        <div className=" w-full md:w-[77.5%] h-screen flex-col  md:mt-[-4px] ml-[-4px] border-4 border-white bg-white box-border overflow-auto">
          ...
          {adminButton === "myProducts" && (
            <div className={` ${productEditMode || commentField ? "hidden" : ""} ml-10 mr-10`}>
              ...
              <div className='w-[12rem] h-10  bg-[#002147] text-white hover:bg-[#c8aa62] border-2 border-[#E7E7E7] pl-11 pt-1 rounded-[10px] md:mt-11 mt-5 mb-5 md:ml-[55rem] ml-[6.5rem] font-serif font-semibold'><button onClick={() => setAdminButton("addNewProduct")} >Add Product</button></div>
              <DataTable
                ...
                <div className='w-full h-[530px] '>
                  <div className='mt-8 '>
                    ...
                    <div className='w-full flex md:ml-[1.6rem] mt-7'>
                      ...
                      <div>
                        <input name='newPassword' onChange={handleUserUploadData} type={passwordtype} placeholder='New Password' className='md:w-[20rem] w-[11rem]   border-b-2   px-5  ' />
                        <button onClick={passwordToggle} className='relative md:top-[-1.7rem] top-[-26px] md:right-[-17.5rem] right-[-9rem] text-[1.5rem] '>{showPassword ? <FaRegEye /> : <FaRegEyeSlash />}</button>
                      </div>
                    </div>
          ...
          {productDetailMode && (
            <div className='fixed top-0 left-0  w-full h-screen bg-white overflow-auto'>
              <div className='flex justify-between  mt-4 md:mt-2 md:px-5 px-3'>
                <h1 className='text-[#002147] text-[1.5rem] md:text-[2rem] font-serif font-semibold text-center   md:leading-10'>Product Details</h1>
          ...
          {adminButton === "allProducts" && (
            <div className={` ${productEditMode || commentField ? "hidden" : ""} md:ml-5 md:mr-10 px-5 md:px-0`}>
              <div>
                <div className="">
                  <div className="flex flex-col md:w-[40rem] md:ml-[1rem] ">
                    {allProducts ? <h1 className="text-[#002147]  mt-5  md:text-[2.5rem] text-[2rem] font-bold font-serif">All Products</h1> : ''}
                    <div>
                      {
                        allProducts ? (
                          allProducts.map((data, index) => (
                            <>
                              <div key={index} className="w-full flex-col mt-11">
                                <div className='flex justify-between '>
                                ...
                                <div className=' min-h-[3rem]   px-1 pt-2 border-2 '>{data.productDescription}</div>
                                <div className=' flex justify-between  py-2 border-b-2 mx-3 '>
