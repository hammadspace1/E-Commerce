...
export default function Products() {
  ...
  const fetchProducts = async () => {
    setIsLoading(true);
    try {
      const { data } = await axios.get("http://localhost:3000/api/productUpload");
  ...
  return (
    <div className='mt-16 md:mt-20'>
      ...
      <div className="">
        <div className="flex flex-col md:w-[40rem] md:ml-[27rem] ">
          {allProducts ? <h1 className='text-[#002147] font-serif mt-3 md:mt-0 text-[2rem] text-center font-bold '>Our Products</h1> : ''}
          <div>
            {
              allProducts ? (
                allProducts.map((data, index) => (
                  <>
                    <div key={index} className="w-full flex-col cursor-pointer mt-11">
